...
class DocumentDataset(Dataset):
	...
	def __init__(self, path, for_model='oag-bert-v1'):
		...
		if for_model == 'oag-bert-v1':
			self.sequence = [str(papers[id]['title']) + '.' + self.parse_abstract(papers[id]['abstract']) for id in
							 self.ids]
		elif for_model == 'specter':
			self.sequence = [
				str(papers[id]['title']) + '[SEP]' + (self.parse_abstract(papers[id].get('abstract')) or '') for id in
				self.ids]
		else:
			raise ('No such model')
...
class ONNXModel():
	...
	def __init__(self, path_to_onnx, tokenizer_pretrained_model, tokenizer_max_length, inputs, is_pooled_output):
		"""Initiliaze model"""
		self.model = onnxruntime.InferenceSession(path_to_onnx,
												  providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
		self.tokenizer_max_length = tokenizer_max_length
		self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_pretrained_model)
		self.inputs = inputs
		self.output = is_pooled_output
	...
	def forward(self, text):
		...
		if self.output:
			return self.model.run(None, ort_inputs)[0].squeeze(0).tolist()
		else:
